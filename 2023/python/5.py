    ...
    while k < len(lines):
        if not lines[k]:
            ...
            d_start, s_start, length = [
                int(i.group()) for i in re.finditer(r"(\d+)", lines[k])
            ]
...
def part2(lines):
    # construct data structures as needed
    seed_ranges = [
        (range(int(i[0]), int(i[0]) + int(i[1])))
        for i in [i.split() for i in re.findall(r"(\d+\s\d+)", lines[0])]
    ]
    lines = lines[3:]
    k = 0
                        ...
                        if r_b >= range_b_b and r_e <= range_b_e:
                            ...
                            if r_e > range_b_e:
                                ...
                                if processed != -1:
                                    ...
                                    seed_ranges[sr] = range(
                                        r_b + diff, range_b_e + diff
                                    )
                                    processed[sr] = 1
